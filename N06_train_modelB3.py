...
from N06_modelB3 import get_model
from N06_serverB3 import client_generator
...
if __name__=="__main__":
  ...
  filepath = "./saved_model/yuvB3-BestWeights.hdf5"
  ...
  history = model.fit(
    ...
    steps_per_epoch=16, epochs=5,
    validation_steps=20*1200/25., verbose=1, callbacks=callbacks_list)

  print('#--- # of epochs are run =', len(history.history['loss']))
  model.save('./saved_model/yuvB3.h5')

  np.save('./saved_model/yuvB3_loss', np.array(history.history['loss']))
  lossnpy = np.load('./saved_model/yuvB3_loss.npy')
  plt.plot(lossnpy)
